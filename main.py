import ctypes,json,os,platform,subprocess
...
def sysinfo():
    o,_,_=ps("Get-CimInstance Win32_OperatingSystem | Select Caption,BuildNumber | ConvertTo-Json -Compress")
    try:d=json.loads(o); osn=d.get('Caption','Windows'); build='Build '+str(d.get('BuildNumber',''))
    except:osn='Windows';build=''
    o,_,_=ps("Get-CimInstance Win32_Processor | Select -First 1 -Expand Name"); cpu=o.splitlines()[0] if o else platform.processor() or 'Unknown'
    o,_,_=ps("(Get-CimInstance Win32_ComputerSystem).TotalPhysicalMemory")
    try:ram=f'{int(o)/(1024**3):.1f} GB'
    except:ram='Unknown'
    gs=gpus(); gpu=gs[0]['name'] if gs else 'Unknown'
    if len(gs)>1:gpu+=f' + {len(gs)-1} more'
    return osn,build,cpu,ram,gpu
...
class App(ctk.CTk):
    ...
    def p_dashboard(self):
        ...
        self.refresh()
    def refresh(self):
        if not hasattr(self,'labels'):return
        osn,build,cpu,ram,gpu=sysinfo(); self.labels['os'].configure(text=osn+'\n'+build); self.labels['cpu'].configure(text=cpu); self.labels['ram'].configure(text=ram); self.labels['gpu'].configure(text=gpu)
    ...
    def generic(self,t,sub,cmd,height=1):
        self.title(t,sub); out=ctk.CTkTextbox(self.content,fg_color=PANEL,border_color=BORDER,border_width=1,corner_radius=12,text_color=T,font=('Consolas',10)); out.pack(fill='both',expand=True,padx=30,pady=10)
        def scan():
    ...
    def p_dns(self):
        self.page_title('DNS','Displays configured IPv4 DNS servers.'); o,e,c=ps("Get-DnsClientServerAddress -AddressFamily IPv4 | Where ServerAddresses | Select InterfaceAlias,ServerAddresses | Format-Table -AutoSize")
